...
var forcing = !1, ua = navigator.userAgent.toLowerCase(), is_android = 0 <= ua.indexOf("android"), uua = navigator.userAgent, is_ios = 0 <= uua.indexOf("iPad") || 0 <= uua.indexOf("iPhone") || 0 <= uua.indexOf("iPod"), is_mobile = 0 <= ua.indexOf("mobile"), is_firefox = -1 < ua.indexOf("firefox"), is_ie8oo = window.attachEvent && !window.addEventListener, is_safari = is_firefox = !1;
...
is_mobile || (no_raf = !0);
var no_raf = !0, a, i, j, k, l, m, n, o, r, fj, d, d2, qq, sc, agpu = "translateZ(0)", ang, sang;
function trf(b, h) {
...
var pi2 = 2 * Math.PI, animating = !1, startAnimation = function () {
    ...
    wic++;
    var h = {}, c = document.createElement("img");
    h.ii = c;
    h.sc = 1;
    c.onload = function () {
        for (var c = ois.length - 1; 0 <= c; c--)if (ois[c].ii == this) {
            c = ois[c];
            c.ww = this.width;
            c.hh = this.height;
            c.loaded = !0;
...
imgd = map.data;
l = imgd.length;
for (yy = p = 0; yy < grh; yy++)for (j = (grh - 1 - yy) / (grh - 1), j = .5 * (1 - Math.cos(Math.PI * j)), xx = 0; xx < grw; xx++)imgd[p] = Math.min(255, Math.floor(54.4 + 32 * j)), imgd[p + 1] = Math.min(255, Math.floor(108.8 + 64 * j)), imgd[p + 2] = Math.min(255, Math.floor(81.6 + 48 * j)), imgd[p + 3] = 255, p += 4;
ctx.putImageData(map, 0, 0);
sadg = elem.toDataURL();
elem = document.createElement("canvas");
elem.width = grw;
...
imgd = map.data;
l = imgd.length;
for (yy = p = 0; yy < grh; yy++)for (j = (grh - 1 - yy) / (grh - 1), j = .5 * (1 - Math.cos(Math.PI * j)), xx = 0; xx < grw; xx++)imgd[p] = Math.min(255, Math.floor(72 + .95 * 48 * j)), imgd[p + 1] = Math.min(255, Math.floor(171 + 93.1 * j)), imgd[p + 2] = Math.min(255, Math.floor(132 + .95 * 87 * j)), imgd[p + 3] = 255, p += 4;
ctx.putImageData(map, 0, 0);
sadu = elem.toDataURL();
elem = document.createElement("canvas");
elem.width = grw;
...
imgd = map.data;
l = imgd.length;
for (yy = p = 0; yy < grh; yy++)for (j = yy / (grh - 1), j = .5 * (1 - Math.cos(Math.PI * j)), xx = 0; xx < grw; xx++)imgd[p] = Math.floor(.1 * 48 + 36 * j), imgd[p + 1] = Math.floor(7 + 52.5 * j), imgd[p + 2] = Math.floor(6.4 + 48 * j), imgd[p + 3] = 255, p += 4;
ctx.putImageData(map, 0, 0);
sadd = elem.toDataURL();
...
imgd = map.data;
l = imgd.length;
for (yy = p = 0; yy < grh; yy++)for (j = (grh - 1 - yy) / (grh - 1), j = .5 * (1 - Math.cos(Math.PI * j)), xx = 0; xx < grw; xx++)imgd[p] = Math.min(255, Math.floor(.85 * 52 + 26 * j)), imgd[p + 1] = Math.min(255, Math.floor(81.6 + 48 * j)), imgd[p + 2] = Math.min(255, Math.floor(.85 * 144 + 72 * j)), imgd[p + 3] = 255, p += 4;
ctx.putImageData(map, 0, 0);
sadg = elem.toDataURL();
elem = document.createElement("canvas");
elem.width = grw;
...
imgd = map.data;
l = imgd.length;
for (yy = p = 0; yy < grh; yy++)for (j = (grh - 1 - yy) / (grh - 1), j = .5 * (1 - Math.cos(Math.PI * j)), xx = 0; xx < grw; xx++)imgd[p] = Math.min(255, Math.floor(72 + .95 * 48 * j)), imgd[p + 1] = Math.min(255, Math.floor(132 + .95 * 87 * j)), imgd[p + 2] = Math.min(255, Math.floor(171 + 93.1 * j)), imgd[p + 3] = 255, p += 4;
ctx.putImageData(map, 0, 0);
sadu = elem.toDataURL();
elem = document.createElement("canvas");
elem.width = grw;
...
imgd = map.data;
l = imgd.length;
for (yy = p = 0; yy < grh; yy++)for (j = yy / (grh - 1), j = .5 * (1 - Math.cos(Math.PI * j)), xx = 0; xx < grw; xx++)imgd[p] = Math.floor(.1 * 48 + 36 * j), imgd[p + 1] = Math.floor(5.4 + 40.5 * j), imgd[p + 2] = Math.floor(7 + 52.5 * j), imgd[p + 3] = 255, p += 4;
ctx.putImageData(map, 0, 0);
sadd = elem.toDataURL();
    ...
    b.onmouseenter = function () {
        for (var c = mos.length - 1; 0 <= c; c--) {
            ...
            if (b.elem == this) {
                if (!b.disabled && !b.mo) {
                    b.mo = !0;
                    if (b.onmouseenter)b.onmouseenter();
    ...
    b.onmouseleave = function () {
        for (var c = mos.length - 1; 0 <= c; c--) {
            ...
            if (b.elem == this) {
                if (b.mo) {
                    b.mo = !1;
                    if (b.onmouseleave)b.onmouseleave();
                    -1 == m_iv && (m_iv = setInterval("hmos()", 25))
                }
                break
            }
    ...
    b.onmousedown = function (c) {
        for (var b =
            ...
            if (q.elem == this) {
                if (!q.disabled && !q.md) {
                    q.md = !0;
                    if (q.onmousedown)q.onmousedown(c, q);
                    -1 == m_iv && (m_iv = setInterval("hmos()", 25));
                    return !1
                }
                break
        ...
        for (var b = mos.length - 1; 0 <= b; b--) {
            ...
            if (q.elem == this) {
                if (q.md) {
                    q.mdf = 1;
                    q.md = !1;
                    if (q.onmouseup && (q.onmouseup(c, q), is_mobile))q.elem.onmouseleave();
                    -1 == m_iv && (m_iv = setInterval("hmos()", 25))
                }
                break
            }
...
function makeTextBtn(b, h, c, f, C) {
    ...
    e.borderRadius = f + "px";
    var E = document.createElement("div"), e = E.style;
    e.position = "absolute";
    e.left = e.top = "0px";
    e.width = y + "px";
    e.height = h + "px";
    ...
    e.opacity = 0;
    E.className = "sadu" + C;
    var u = document.createElement("div"), e = u.style;
    e.position = "absolute";
    e.left = e.top = "-1px";
    e.width = y + 2 + "px";
    e.height = h + 2 + "px";
...
function loginFade() {
    var b = Date.now(), h = (b - llgmtm) / 25;
    llgmtm = b;
    login_fr += .05 * h;
    choosing_skin && (login_fr += .06 * h);
    1 <= login_fr ? (login_fr = 1, login.style.display = "none", cstx.style.display = "none", fbh.style.display = "none", twth.style.display = "none", cskh.style.display = "none", plq.style.display = "none", clq.style.display = "none", login.style.opacity = 1, cstx.style.opacity = 1, fbh.style.opacity = 1, twth.style.opacity = 1, cskh.style.opacity = 1, plq.style.opacity = 1, clq.style.opacity = 1, pskh.style.opacity = 1, nskh.style.opacity =
        1, skodiv.style.opacity = 1, tip_fr = -1, tips.style.display = "none", mc.style.opacity = 1, loch.style.opacity = 1, clearInterval(login_iv), login_iv = -1, -1 != showlogo_iv && (ncka = lgss = lga = 1, showLogo(!0), -1 != showlogo_iv && (clearInterval(showlogo_iv), showlogo_iv = -1))) : (lgcsc = 1 + .1 * Math.pow(login_fr, 2), b = Math.round(lgbsc * lgcsc * 1E5) / 1E5, trf(login, "scale(" + b + "," + b + ")"), login.style.opacity = 1 - login_fr, cstx.style.opacity = 1 - login_fr, fbh.style.opacity = 1 - login_fr, twth.style.opacity = 1 - login_fr, cskh.style.opacity = 1 - login_fr, plq.style.opacity =
...
var sko_btn = o = makeTextBtn(String.fromCharCode(160) + "Save" + String.fromCharCode(160), 47, 20, 34, 1), skodiv = o.elem;
...
var play_btn = o = makeTextBtn(String.fromCharCode(160) + "Play" + String.fromCharCode(160), 47, 20, 34, 1), pbdiv = o.elem;
...
var tips = document.getElementById("tips"), tipss = ["Eat to grow longer!", "Don't run into other snakes!", "When longer, hold the mouse for a speed boost!"], tip_pos = -1, tip_fr = 1.9;
o.elem.onclick = function () {
...
o.elem.onclick = function () {
    if (!save_btn.disabled) {
        ...
        } else for (h = new Uint8Array(1 + b.length), h[0] = 118, c = 0; c < b.length; c++)h[c + 1] = b.charCodeAt(c);
        ws.send(h);
...
e1.width = 80;
e1.height = 850;
var ectx = e1.getContext("2d"), ep = ectx.createPattern(grad, "repeat");
ectx.fillStyle = ep;
...
var bpx1, bpy1, bpx2, bpy2, fpx1, fpy1, fpx2, fpy2, sgsc = .9, gsc = sgsc, nsep = 4.5, tasty = 0, rr, gg, bb, render_mode = 2;
...
function setMscps(b) {
    if (b != mscps) {
        ...
        var h = fmlts[fmlts.length - 1], c = fpsls[fpsls.length - 1];
...
var pbx = new Float32Array(32767), pby = new Float32Array(32767), pba = new Float32Array(32767), pbu = new Uint8Array(32767), per_color_imgs = [], rrs = [192, 144, 128, 128, 238, 255, 255, 255, 224, 255, 144, 80, 255, 40, 100, 120, 72, 160, 255, 56, 56], ggs = [128, 153, 208, 255, 238, 160, 144, 64, 48, 255, 153, 80, 192, 136, 117, 134, 84, 80, 224, 68, 68], bbs = [255, 255, 208, 128, 112, 96, 144, 64, 224, 255, 255, 80, 80, 96, 255, 255, 255, 255, 64, 255, 255], max_skin_cv = 21;
...
if (testing)for (ctx = colc.getContext("2d"), ctx.fillStyle = "#000000", ctx.fillRect(0, 0, colc.width, colc.height), ctx.fillStyle = "#FFFFFF", ctx.font = "10px Arial, Helvetica Neue, Helvetica, sans-serif", ctx.textBaseline = "top", ctx.textAlign = "center", i = yy = xx = 0; i < rrs.length; i++) {
...
var view_xx = 0, view_yy = 0, view_ang = 0, view_dist = 0, fvx = 0, fvy = 0, xm = 0, ym = 0, lsxm = 0, lsym = 0, snake = null, dhx, dhy, hsz, fr = 0, lfr = 0, ltm = Date.now(), vfr = 0, vfrb = 0, fr2 = 0, lfr2 = 0, vfrb2 = 0, cptm = 0, lptm = 0, lpstm = 0, last_ping_mtm = 0, lagging = !1, lag_mult = 1, wfpr = !1, high_quality = !0, gla = 1, wdfg = 0, qsm = 1, playing = !1, connected = !1, want_close_socket = !1, want_victory_message = !1, want_victory_focus = !1, want_hide_victory = 0, hvfr = 0, dead_mtm = -1, at2lt = new Float32Array(65536);
for (yy = 0; 256 > yy; yy++)for (xx = 0; 256 > xx; xx++)at2lt[yy << 8 | xx] = Math.atan2(yy - 128, xx - 128);
var kd_l_frb = 0, kd_r_frb = 0, kd_l = !1, kd_r = !1, kd_u = !1, lkstm = 0, oef = function () {
    var b = Date.now();
    vfr = (b - ltm) / 8;
    ltm = b;
    ...
    if (-1 != play_btn_click_mtm &&
        ...
        var h = document.createElement("img"), c = [];
        c.push("bso=" + encodeURIComponent(bso));
        if (bso)for (var f in bso)c.push(f + "=" + encodeURIComponent(bso[f]));
    ...
    if (choosing_skin) {
        for (h = snakes.length - 1; 0 <= h; h--)for (c = snakes[h], f = c.pts.length - 1; 0 <= f; f--)c.pts[f].yy = grd / 2 + 15 * Math.cos(f / 4 + fr / 19) * (1 - f / c.pts.length);
        view_xx -= vfr
    }
    ...
    if (-1 == dead_mtm)-1 != lb_fr && 1 != lb_fr && (lb_fr += .01 * vfr, 1 <= lb_fr && (lb_fr = 1), lbh.style.opacity = .85 * lb_fr, lbs.style.opacity = lbn.style.opacity = lbp.style.opacity = lbf.style.opacity = vcm.style.opacity = lb_fr); else if (1600 < b - dead_mtm) {
        if (-1 == login_iv) {
            login_iv = -2;
    ...
    var C;
    if (!choosing_skin)for (h = snakes.length - 1; 0 <= h; h--) {
        c = snakes[h];
        ...
        if (!c.dead) {
            ...
            if (0 < c.fltg)for (b = vfrb, b > c.fltg && (b = c.fltg), c.fltg -= b, qq = 0; qq < b; qq++)c.fl = c.fls[c.flpos], c.fls[c.flpos] =
                0, c.flpos++, c.flpos >= lfc && (c.flpos = 0); else 0 == c.fltg && (c.fltg = -1, c.fl = 0);
            c.cfl = c.tl + c.fl
        }
        if (1 == c.dir) {
            c.ang -= C;
        ...
        if (0 < vfrb) {
            ...
            for (f = c.pts.length - 1; 0 <= f; f--)if (q = c.pts[f], 0 < q.eiu) {
                fy = fx = 0;
        ...
        c.rey > f && (c.rey -= vfr / 6, c.rey <= f && (c.rey = f));
        if (0 < vfrb) {
            if (0 < c.ftg)for (b = vfrb, b > c.ftg && (b = c.ftg), c.ftg -= b, qq = 0; qq < b; qq++)c.fx = c.fxs[c.fpos], c.fy = c.fys[c.fpos], c.fchl = c.fchls[c.fpos], c.fxs[c.fpos] =
                0, c.fys[c.fpos] = 0, c.fchls[c.fpos] = 0, c.fpos++, c.fpos >= rfc && (c.fpos = 0); else 0 == c.ftg && (c.ftg = -1, c.fx = 0, c.fy = 0, c.fchl = 0);
            if (0 < c.fatg)for (b = vfrb, b > c.fatg && (b = c.fatg), c.fatg -= b, qq = 0; qq < b; qq++)c.fa = c.fas[c.fapos], c.fas[c.fapos] = 0, c.fapos++, c.fapos >= afc && (c.fapos = 0); else 0 == c.fatg && (c.fatg = -1, c.fa = 0)
        }
    ...
    for (h = preys.length - 1; 0 <= h; h--) {
        ...
        f = q.sp * vfr / 4;
        if (0 <
            vfrb)if (0 < q.ftg)for (b = vfrb, b > q.ftg && (b = q.ftg), q.ftg -= b, qq = 1; qq <= b; qq++)qq == b && (q.fx = q.fxs[q.fpos], q.fy = q.fys[q.fpos]), q.fxs[q.fpos] = 0, q.fys[q.fpos] = 0, q.fpos++, q.fpos >= rfc && (q.fpos = 0); else 0 == q.ftg && (q.fx = 0, q.fy = 0, q.ftg = -1);
        if (1 == q.dir) {
            q.ang -= C;
    ...
    if (animating) {
        ...
        if (testing)for (h = sectors.length - 1; 0 <= h; h--)c = sectors[h], b.fillStyle = "rgba(0, 255, 0, .1)", b.fillRect(mww2 + (c.xx * sector_size - view_xx) * gsc, mhh2 + (c.yy * sector_size - view_yy) * gsc, sector_size * gsc - 4, sector_size * gsc - 4);
        ...
        } else for (h = foods_c - 1; 0 <= h; h--)c = foods[h], c.rx >= fpx1 && c.ry >= fpy1 && c.rx <= fpx2 && c.ry <= fpy2 && (1 == c.rad ? (A = mww2 + gsc * (c.rx - view_xx) - c.fw2, t = mhh2 + gsc * (c.ry - view_yy) - c.fh2, -50 <= A && -50 <= t && A <= mwwp50 && t <= mhhp50 && (b.globalAlpha = c.fr, b.drawImage(c.fi, A, t))) : (A = mww2 + gsc * (c.rx - view_xx) - c.fw2 * c.rad, t = mhh2 + gsc * (c.ry - view_yy) - c.fh2 * c.rad, -50 <=
        A && -50 <= t && A <= mwwp50 && t <= mhhp50 && (b.globalAlpha = c.fr, b.drawImage(c.fi, 0, 0, c.fw, c.fh, A, t, c.fw * c.rad, c.fh * c.rad))));
        b.restore();
        b.save();
        b.globalCompositeOperation = "lighter";
        for (h = preys.length - 1; 0 <= h; h--)if (f = preys[h], e = f.xx + f.fx, y = f.yy + f.fy, px = mww2 + gsc * (e - view_xx), py = mhh2 + gsc * (y - view_yy), -50 <= px && -50 <= py && px <= mwwp50 && py <= mhhp50) {
            if (f.eaten) {
                var c = f.eaten_by, q = Math.pow(f.eaten_fr, 2), e = e + (c.xx + c.fx + Math.cos(c.ang + c.fa) * (43 - 24 * q) * (1 - q) - e) * q, y = y + (c.yy + c.fy + Math.sin(c.ang + c.fa) * (43 - 24 * q) * (1 - q) - y) * q;
                px =
                    mww2 + gsc * (e - view_xx);
        ...
        for (h = snakes.length - 1; 0 <= h; h--)for (c = snakes[h], c.iiv = !1, x = c.pts.length - 1; 0 <= x; x--)if (e = c.pts[x], px = e.xx + e.fx, py = e.yy + e.fy, px >= bpx1 && py >= bpy1 && px <= bpx2 && py <= bpy2) {
            c.iiv = !0;
            break
        }
        for (h = snakes.length - 1; 0 <= h; h--)if (c = snakes[h], c.iiv) {
            f = c.xx + c.fx;
            C = c.yy + c.fy;
            px = f;
            py = C;
            E = c.ehang;
            var u = c.sc, w = 29 * u, D = c.cfl;
            ...
            if (2 == render_mode) {
                ...
                for (var N = c.sep *
                    qsm, J = 0, t = per_color_imgs[c.cv].kmcs, F, G, x = c.pts.length - 1; 0 <= x; x--)if (e = c.pts[x], lpx = px, lpy = py, px = e.xx + e.fx, py = e.yy + e.fy, -.25 < D) {
                    L = B;
                    I = K;
                    B = (px + lpx) / 2;
                    K = (py + lpy) / 2;
                ...
                if (c.tsp > c.fsp) {
                    ...
                    b.save();
                    b.globalCompositeOperation = "lighter";
                    if (c.rbcs)for (K = c.rbcs, N = K.length, x = A - 1; 0 <= x; x--)1 == pbu[x] && (px = pbx[x], py = pby[x], e = per_color_imgs[K[x % N]], e = e.kfmc, b.save(), b.globalAlpha = D * y * .38 * (.6 + .4 * Math.cos(x / 4 - 1.15 * c.sfr)), b.translate((px - view_xx) * gsc, (py - view_yy) * gsc), b.drawImage(e, -J, -J, 2 * J, 2 * J), b.restore()); else for (x = A - 1; 0 <= x; x--)1 == pbu[x] && (px = pbx[x], py =
                ...
                F *= D;
                if (high_quality || 0 < gla)for (e = F, 1 != gla && (e = F * gla), b.globalAlpha = e, x = A - 1; 0 <= x; x--)1 == pbu[x] && (px = pbx[x], py = pby[x], b.save(), b.translate((px - view_xx) * gsc, (py - view_yy) * gsc), b.drawImage(komc, -q, -q, 2 * q, 2 * q), 9 > x && (b.globalAlpha = e * (1 - x / 9), b.drawImage(ksmc, -B, -B, 2 * B, 2 * B), b.globalAlpha = e), b.restore());
                b.globalAlpha = F;
                if (c.rbcs) {
                    K =
                        c.rbcs;
                ...
                if (c.antenna && 2 <= A && 1 == pbu[1]) {
                    ...
                    F = c.sc * gsc;
                    fj = c.atx.length - 1;
                    if (choosing_skin)for (x = 1; x <= fj; x++)c.atvx[x] -= .3, c.atvy[x] += .14 * Math.cos(fr / 23 - 7 * x / fj);
        ...
        wumsts && 0 < rank && 0 < snake_count && playing && (wumsts = !1, h = "" + ('<span style="font-size: 14px;"><span style="opacity: .4;">Your length: </span><span style="opacity: .8; font-weight: bold;">' +
            Math.floor(150 * (fpsls[snake.sct] + snake.fam / fmlts[snake.sct] - 1) - 50) / 10 + "</span></span>"), h += '<BR><span style="opacity: .3;">Your rank: </span><span style="opacity: .35;">' + rank + '</span><span style="opacity: .3;"> of </span><span style="opacity: .35;">' + snake_count + "</span>", lbf.innerHTML = h);
        b.restore()
    }
...
function resize() {
    ...
    if (ww != lww || hh != lhh) {
        ...
        var h = Math.sqrt(ww * ww + hh * hh), b = Math.ceil(1400 * ww / h), c = Math.ceil(1400 * hh / h);
...
if (testing)for (i = 0; 256 > i; i++)rdpspc[i] = -1;
...
function connect() {
    if (0 == sos.length)waiting_for_sos || (waiting_for_sos = !0, sos_ready_after_mtm = -1); else {
        waiting_for_sos = !1;
        ...
        if (!forcing) {
            ...
            if ("undefined" != typeof rmsos)for (b = 0; b < rmsos.length; b++)for (h =
        ...
        ws.onmessage = function (c) {
            if (ws == this && (c = new Uint8Array(c.data),
                ...
                lptm = cptm;
                cptm = Date.now();
                var b = c[0] << 8 | c[1], e = cptm - lptm;
                0 == lptm && (e = 0);
                etm += e - b;
                testing && (rdpspc[c[2]] += c.length);
                var f = String.fromCharCode(c[2]), b = 3, h = c.length, e = c.length - 2, u = c.length - 3;
                if ("a" == f)connecting = !1, playing = connected = !0, play_btn_click_mtm = -1, grd = c[b] << 16 | c[b + 1] << 8 | c[b + 2], b += 3, e = c[b] << 8 | c[b + 1], b += 2, sector_size = c[b] << 8 | c[b + 1], b += 2, sector_count_along_edge = c[b] << 8 | c[b + 1], b += 2, spangdv = c[b] / 10, b++, nsp1 = (c[b] << 8 | c[b + 1]) / 100, b += 2, nsp2 = (c[b] << 8 | c[b + 1]) / 100, b += 2, nsp3 =
                    (c[b] << 8 | c[b + 1]) / 100, b += 2, mamu = (c[b] << 8 | c[b + 1]) / 1E3, b += 2, mamu2 = (c[b] << 8 | c[b + 1]) / 1E3, b += 2, cst = (c[b] << 8 | c[b + 1]) / 1E3, b += 2, b < h && (protocol_version = c[b]), console.log("game radius = " + grd), setMscps(e), lbh.style.display = "inline", lbs.style.display = "inline", lbn.style.display = "inline", lbp.style.display = "inline", lbf.style.display = "inline", vcm.style.display = "inline", loch.style.display = "inline", startShowGame(); else if ("e" == f || "E" == f || "3" == f || "4" == f || "5" == f) {
                    var w = c[b] << 8 | c[b + 1], b = b + 2, D = h = -1, x = -1, A = -1;
                    if (6 <= protocol_version)6 ==
                    e ? (h = "e" == f ? 1 : 2, D = 2 * c[b] * Math.PI / 256, b++, x = 2 * c[b] * Math.PI / 256, b++, A = c[b] / 18) : 5 == e ? "e" == f ? (D = 2 * c[b] * Math.PI / 256, b++, A = c[b] / 18) : "E" == f ? (h = 1, x = 2 * c[b] * Math.PI / 256, b++, A = c[b] / 18) : "4" == f ? (h = 2, x = 2 * c[b] * Math.PI / 256, b++, A = c[b] / 18) : "3" == f ? (h = 1, D = 2 * c[b] * Math.PI / 256, b++, x = 2 * c[b] * Math.PI / 256) : "5" == f && (h = 2, D = 2 * c[b] * Math.PI / 256, b++, x = 2 * c[b] * Math.PI / 256) : 4 == e && ("e" == f ? D = 2 * c[b] * Math.PI / 256 : "E" == f ? (h = 1, x = 2 * c[b] * Math.PI / 256) : "4" == f ? (h = 2, x = 2 * c[b] * Math.PI / 256) : "3" == f && (A = c[b] / 18)); else if (3 <= protocol_version) {
                        "3" != f &&
                    ...
                    var w = c[b] << 8 | c[b + 1], b = b + 2, H = (c[b] << 16 | c[b + 1] << 8 | c[b + 2]) / 16777215;
                    if (t = os["s" + w])t.fam = H, snl(t)
                } else if ("r" == f) {
                    if (w = c[b] << 8 | c[b + 1], b += 2, t = os["s" +
                        w]) {
                        4 <= u && (t.fam = (c[b] << 16 | c[b + 1] << 8 | c[b + 2]) / 16777215);
                        for (u = 0; u < t.pts.length; u++)if (!t.pts[u].dying) {
                            t.pts[u].dying = !0;
                            t.sct--;
                    ...
                    if (playing && (H = "n" == f || "N" == f, w = c[b] << 8 | c[b + 1], b += 2, t = os["s" + w])) {
                        if (H)t.sct++; else for (u = 0; u < t.pts.length; u++)if (!t.pts[u].dying) {
                            t.pts[u].dying = !0;
                            break
                        }
                        ...
                        (z = points_dp.get()) || (z = {exs: [], eys: [], efs: [], ems: []});
                        z.eiu = 0;
                        z.xx = e;
                        z.yy = B;
                        z.fx = 0;
                        ...
                        w = t.pts.length - 3;
                        if (1 <= w)for (D = t.pts[w], f = n = 0, u = w - 1; 0 <= u; u--)w = t.pts[u], n++, c = w.xx, b = w.yy, 4 >= n && (f = cst * n / 4), w.xx += (D.xx - w.xx) * f, w.yy += (D.yy - w.yy) * f, t.iiv && (c -= w.xx, b -= w.yy, w.fx += c, w.fy += b, w.exs[w.eiu] = c, w.eys[w.eiu] = b, w.efs[w.eiu] = 0, w.ems[w.eiu] = 2, w.eiu++), D = w;
                        t.sc = Math.min(6, 1 + (t.sct - 2) / 106);
                        t.scang = .13 + .87 * Math.pow((7 - t.sc) / 6, 2);
                        t.ssp = nsp1 + nsp2 * t.sc;
                        t.fsp = t.ssp + .1;
                    ...
                    if (playing) {
                        ...
                        for (b += 2; b < h;) {
                            var I = c[b] << 8 | c[b + 1], b = b + 2, H = (c[b] << 16 | c[b + 1] << 8 | c[b + 2]) / 16777215, b = b + 3, t = c[b] % 9;
                            b++;
                            e = c[b];
                            b++;
                            f = "";
                    ...
                    "none", smh.style.display = "none", victory_holder.style.display = "inline", saveh.style.display = "block", want_victory_focus = want_victory_message = !0, victory.disabled = !1, save_btn.setEnabled(!0)) : want_close_socket = !0); else if ("w" == f)if (h = c[b], b++, e = c[b] << 8 | c[b + 1], b += 2, B = c[b] << 8 | c[b + 1], 1 == h)t = {}, t.xx = e, t.yy = B, sectors.push(t); else {
                ...
                } else if ("p" == f)wfpr = !1, lagging && (etm *= lag_mult, lagging = !1); else if ("u" == f) {
                    u = asmc.getContext("2d");
                    u.clearRect(0, 0, 80, 80);
                    u.fillStyle = "#FFFFFF";
                    for (var B =
                        e = 0; b < h && !(80 <= B);)if (w = c[b++], 128 <= w)for (w -= 128, z = 0; z < w && !(e++, 80 <= e && (e = 0, B++, 80 <= B)); z++); else for (z = 0; 7 > z && !(0 < (w & u_m[z]) && u.fillRect(e, B, 1, 1), e++, 80 <= e && (e = 0, B++, 80 <= B)); z++);
                } else if ("s" == f) {
                    if (playing)if (w = c[b] << 8 | c[b + 1], b += 2, 6 < u) {
                        ...
                        t = c[b];
                        b++;
                        var L = [], I = (c[b] << 16 | c[b + 1] << 8 | c[b + 2]) / 5, b = b + 3, K = (c[b] << 16 | c[b + 1] << 8 |
                            c[b + 2]) / 5, b = b + 3, e = c[b];
                        b++;
                        f = "";
                        ...
                        t.sep = t.wsep;
                        snl(t)
                    } else for (c = 1 == c[b], z = snakes.length - 1; 0 <= z; z--)if (snakes[z].id == w) {
                        snakes[z].id = -1234;
                        c ? (snakes[z].dead = !0, snakes[z].dead_amt = 0, snakes[z].edir = 0) : snakes.splice(z, 1);
                        delete os["s" + w];
                        break
                    }
                } else if ("F" == f)if (4 <= protocol_version) {
                    ...
                    t.yy = H;
                    sectors.push(t)
                } else for (z = c[b] << 8 | c[b + 1], b += 2, H = c[b] << 8 | c[b + 1], b += 2, t = {}, t.xx = z, t.yy = H, sectors.push(t); b < h;)w = c[b] << 16 | c[b + 1] << 8 | c[b + 2], b += 3, t = c[b], b++, e = sector_size * (z + c[b] / 255), b++, B = sector_size * (H + c[b] / 255), b++, u = c[b] / 5, b++, u = newFood(w, e, B, u, !0, t), u.sx = z, u.sy = H; else if ("b" == f || "f" == f)4 <= protocol_version ? (t = c[b], b++, 4 < u && (e = c[b] << 8 | c[b + 1], b += 2, B = c[b] << 8 | c[b + 1],
                    w = B * grd * 3 + e, u = c[b + 2] / 5, u = newFood(w, e, B, u, "b" == f, t), u.sx = Math.floor(e / sector_size), u.sy = Math.floor(B / sector_size))) : (w = c[b] << 16 | c[b + 1] << 8 | c[b + 2], b += 3, 4 < u && (t = c[b], b++, z = c[b] << 8 | c[b + 1], b += 2, H = c[b] << 8 | c[b + 1], b += 2, e = sector_size * (z + c[b] / 255), b++, B = sector_size * (H + c[b] / 255), b++, u = c[b] / 5, u = newFood(w, e, B, u, "b" == f, t), u.sx = z, u.sy = H)); else if ("c" == f) {
                    4 <= protocol_version ? (e = c[b] << 8 | c[b + 1], b += 2, B = c[b] << 8 | c[b + 1], b += 2, w = B * grd * 3 + e) : (w = c[b] << 16 | c[b + 1] << 8 | c[b + 2], b += 3);
                    for (z = cm1 = foods_c - 1; 0 <= z; z--)if (u = foods[z], u.id ==
                        w) {
                        u.eaten = !0;
                    ...
                    b += 2;
                    h = null;
                    for (z = preys.length - 1; 0 <= z; z--)if (preys[z].id == w) {
                        h = preys[z];
                        break
                    }
                    if (h) {
                ...
                } else if ("y" == f)if (w = c[b] << 8 | c[b + 1], b += 2, 2 == u)for (z = preys.length - 1; 0 <= z; z--) {
                    if (h = preys[z], h.id == w) {
                        preys.splice(z, 1);
                        break
                    }
                } else if (4 == u)for (c = c[b] << 8 | c[b + 1], z = preys.length - 1; 0 <= z; z--) {
                    if (h = preys[z], h.id == w) {
                        h.eaten = !0;
                        h.eaten_by = os["s" + c];
...
function asciize(b) {
    ...
    c = b.length;
    var C = !1;
    for (h = 0; h < c; h++)if (f = b.charCodeAt(h), 32 > f || 127 < f) {
        C = !0;
        break
    }
    if (C) {
...
var nick = document.getElementById("nick"), victory = document.getElementById("victory"), victory_bg = document.getElementById("victory_bg"), logo = document.getElementById("logo"), login = document.getElementById("login"), lastscore = document.getElementById("lastscore"), nick_holder = document.getElementById("nick_holder"), victory_holder = document.getElementById("victory_holder"), smh = document.getElementById("smh"), cstx = document.getElementById("cstx");
try {
...
var twth = document.getElementById("twth"), csk = document.getElementById("csk"), cskh = document.getElementById("cskh"), plq = document.getElementById("plq"), clq = document.getElementById("clq");
try {
...
lts.push({
    ...
    pts: [243, 94, 268, 94, 293, 94], kc: 66, ws: 4, wr: .015, qm: .025,
    sp: .06, sz: 9, r: 1.2
}, {
    ...
    sp: .06,
    sz: 11
}, {pts: [663, 177, 663, 178.5, 663, 180], kc: 11, ws: 2, wr: .06, qm: .05, sp: .06, sz: 15}, {
    pts: [717,
        96, 717, 140, 717, 184], kc: 33, ws: 4, wr: .06, qm: .05, sp: .06, sz: 11
}, {
...
function showLogo(b) {
    var h = Date.now(), c = (h - lgtm) / 25;
    lgtm = h;
    var f, C, q, e, y, E, u, w, D, x, A, t, H, z, L;
    lgfr += c;
    if (0 == lts[lts.length - 1].mwig && 1 == lga && 1 == lgss && 1 == ncka)clearInterval(showlogo_iv), showlogo_iv = -1; else {
        ...
        for (h = 0; h < lts.length; h++) {
            var I = lts[h], B = I.pts, K = I.kc, M = I.ws, O = I.wr, N = I.qm;
            f = I.sp;
            var J = I.sz;
            L = I.r;
            var F = I.mwig;
...
document.onkeydown = function (b) {
    b = b || window.event;
    b = b.keyCode;
    if (37 == b)kd_l = !0; else if (39 == b)kd_r = !0; else if (38 == b)kd_u = !0, setAcceleration(1); else if (13 == b || 10 == b)if (want_victory_message) {
...
function loadSos(b) {
    if (!forcing && 0 < b.length) {
        ...
        sis = [];
        b.charAt(0);
        for (var h = 1, c = {}, f = 0, c = f = 0, C, q = 0, e = 0, y = [], E = [], u = []; h < b.length;)if (C = (b.charCodeAt(h++) - 97 - e) % 26, 0 > C && (C += 26), q *= 16, q += C, e += 7, 1 == c) {
            if (0 == f)y.push(q), 4 == y.length && f++; else if (1 == f)E.push(q), 3 == E.length && f++; else if (2 == f && (u.push(q), 3 == u.length)) {
                c = {};
        ...
        h; h--) {
            c = 1;
            for (f = sis.length - 1; 0 <= f; f--)if (sis[f].ip == sos[h].ip) {
                c = 0;
                break
            }
            if (1 == c) {
                ...
                sis.push({ip: sos[b].ip, po: y[Math.floor(Math.random() * y.length)], ptms: []})
            }
        }
        for (f = sis.length - 1; 0 <= f; f--)if (c = sis[f], h = null, !(.4 > Math.random())) {
            try {
                h = new WebSocket("ws://" + c.ip + ":" + c.po + "/ptc")
            } catch (w) {
                h = null
            }
            h && (h.binaryType = "arraybuffer", h.onerror = function (b) {
            }, h.onmessage = function (b) {
                b = new Uint8Array(b.data);
                if (1 == b.length && 112 == b[0])for (var c = sis.length - 1; 0 <= c; c--)if (b = sis[c], b.ps == this) {
                    c = Date.now() - b.stm;
                    b.ptms.push(c);
